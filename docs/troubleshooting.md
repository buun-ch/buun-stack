...
- [Cluster Shutdown and Restart](#cluster-shutdown-and-restart)
...
Always use `just k8s::stop` before shutting down the machine. It drains the node,
stops the k3s service, and terminates remaining containerd-shim processes so
stateful workloads can flush data cleanly:
...
This starts the k3s service, waits for the node to be ready, and uncordons the
node (which was cordoned during drain).
